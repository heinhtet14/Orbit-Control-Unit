...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 /* HAL_ADC_Start_DMA(&hdma_adc, (uint32_t*) adcresultsDMA, adcChannelCount);
	  while (adcConversionComplete == 0)
	  {

	  }
	  adcConversionComplete = 0;
	  snprintf(txbuf, 50, "CH1 = %d\tCH2 = %d\r\n", adcresultsDMA[0], adcresultsDMA[1]);
	  HAL_UART_Transmit(&huart2, (uint8_t*)txbuf, 50, 10);
